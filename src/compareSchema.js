...
var helper = {
	...
	__compareSchemas: function () {
		...
		for (let schema in this.__sourceSchema.schemas) {
			//Get missing schemas on target
			this.__updateProgressbar(this.__progressBarValue + progressBarStep, `Comparing SCHEMA ${schema}`);
			this.__tempScripts = [];

			if (!this.__targetSchema.schemas[schema]) {
	...
	__compareTables: function () {
		...
		if (global.config.options.schemaCompare.dropMissingTable) tablesToCompareLength += Object.keys(this.__targetSchema.tables).length;

		const progressBarStep = 0.1665 / tablesToCompareLength;

		for (let table in this.__sourceSchema.tables) {
			//Get new or changed tables
			...
			if (this.__targetSchema.tables[table]) {
				//Table exists on both database, then compare table schema
				actionLabel = "ALTER";

				//@mso -> relhadoids has been deprecated from PG v12.0
				if (this.__targetSchema.tables[table].hasOwnProperty("options"))
					this.__compareTableOptions(table, this.__sourceSchema.tables[table].options, this.__targetSchema.tables[table].options);

				this.__compareTableColumns(
					table,
					this.__sourceSchema.tables[table].columns,
					this.__targetSchema.tables[table].columns,
					this.__targetSchema.tables[table].constraints,
					this.__targetSchema.tables[table].indexes
				);
		...
		if (global.config.options.schemaCompare.dropMissingTable)
			for (let table in this.__targetSchema.tables) {
				//Get missing tables
				this.__updateProgressbar(this.__progressBarValue + progressBarStep, `Comparing TARGET TABLE ${table}`);
				this.__tempScripts = [];

				if (!this.__sourceSchema.tables.hasOwnProperty(table)) this.__tempScripts.push(sql.generateDropTableScript(table));
	...
	__compareTableColumns: function (table, sourceTableColumns, targetTableColumns, targetTableConstraints, targetTableIndexes) {
		for (let column in sourceTableColumns) {
			//Get new or changed columns
			if (targetTableColumns[column]) {
				//Table column exists on both database, then compare column schema
				this.__compareTableColumn(
					table,
					column,
					sourceTableColumns[column],
					targetTableColumns[column],
					targetTableConstraints,
					targetTableIndexes
				);
			} else {
				...
				if (!global.schemaChanges.newColumns[table]) global.schemaChanges.newColumns[table] = [];

				global.schemaChanges.newColumns[table].push(column);
			}
		}
		for (let column in targetTableColumns) {
			//Get dropped columns
			if (!sourceTableColumns[column])
	...
	__compareTableColumn: function (table, column, sourceTableColumn, targetTableColumn, targetTableConstraints, targetTableIndexes) {
		...
		if (Object.keys(changes).length > 0) {
			...
			for (let constraint in targetTableConstraints) {
				if (this.__droppedConstraints.includes(constraint)) continue;
	...
	__compareTableConstraints: function (table, sourceTableConstraints, targetTableConstraints) {
		...
		for (let constraint in targetTableConstraints) {
			...
			if (!sourceTableConstraints[constraint] && !this.__droppedConstraints.includes(constraint))
	...
	__compareTableIndexes: function (sourceTableIndexes, targetTableIndexes) {
		for (let index in sourceTableIndexes) {
			...
			if (targetTableIndexes[index]) {
				...
				if (sourceTableIndexes[index].definition != targetTableIndexes[index].definition) {
					if (!this.__droppedIndexes.includes(index)) this.__tempScripts.push(sql.generateDropIndexScript(index));
		...
		for (let index in targetTableIndexes) {
			//Get dropped indexes
			if (!sourceTableIndexes[index] && !this.__droppedIndexes.includes(index))
	...
	__compareTablePrivileges: function (table, sourceTablePrivileges, targetTablePrivileges) {
		for (let role in sourceTablePrivileges) {
			...
			if (targetTablePrivileges[role]) {
				...
				if (Object.keys(changes).length > 0) this.__tempScripts.push(sql.generateChangesTableRoleGrantsScript(table, role, changes));
			} else {
	...
	__compareViews: function () {
		...
		if (global.config.options.schemaCompare.dropMissingView) viewsToCompareLength += Object.keys(this.__targetSchema.views).length;

		const progressBarStep = 0.1665 / viewsToCompareLength;

		for (let view in this.__sourceSchema.views) {
			//Get new or changed views
			this.__updateProgressbar(this.__progressBarValue + progressBarStep, `Comparing SOURCE VIEW ${view}`);
			this.__tempScripts = [];
			let actionLabel = "";

			if (this.__targetSchema.views[view]) {
				...
				if (this.__sourceSchema.views[view].definition != this.__targetSchema.views[view].definition) {
					if (!this.__droppedViews.includes(view)) this.__tempScripts.push(sql.generateDropViewScript(view));
		...
		if (global.config.options.schemaCompare.dropMissingView)
			for (let view in this.__targetSchema.views) {
				...
				if (!this.__sourceSchema.views.hasOwnProperty(view)) this.__tempScripts.push(sql.generateDropViewScript(view));
	...
	__compareMaterializedViews: function () {
		...
		if (global.config.options.schemaCompare.dropMissingView) mviewsToCompareLength += Object.keys(this.__targetSchema.materializedViews).length;

		const progressBarStep = 0.1665 / mviewsToCompareLength;

		for (let view in this.__sourceSchema.materializedViews) {
			...
			if (this.__targetSchema.materializedViews[view]) {
				...
				if (this.__sourceSchema.materializedViews[view].definition != this.__targetSchema.materializedViews[view].definition) {
					if (!this.__droppedViews.includes(view)) this.__tempScripts.push(sql.generateDropMaterializedViewScript(view));
					...
					this.__compareTableIndexes(
						this.__sourceSchema.materializedViews[view].indexes,
						this.__targetSchema.materializedViews[view].indexes
					);
					this.__compareTablePrivileges(
						view,
						this.__sourceSchema.materializedViews[view].privileges,
						this.__targetSchema.materializedViews[view].privileges
					);
		...
		if (global.config.options.schemaCompare.dropMissingView)
			for (let view in this.__targetSchema.materializedViews) {
				//Get missing materialized views
	...
	__compareProcedures: function () {
		...
		if (global.config.options.schemaCompare.dropMissingFunction) proceduresToCompareLength += Object.keys(this.__targetSchema.functions).length;

		const progressBarStep = 0.1665 / proceduresToCompareLength;

		for (let procedure in this.__sourceSchema.functions) {
			//Get new or changed procedures
			this.__updateProgressbar(this.__progressBarValue + progressBarStep, `Comparing SOURCE FUNCTION ${procedure}`);
			this.__tempScripts = [];
			let actionLabel = "";

			if (this.__targetSchema.functions[procedure]) {
				...
				if (this.__sourceSchema.functions[procedure].definition != this.__targetSchema.functions[procedure].definition) {
					...
					this.__compareProcedurePrivileges(
						procedure,
						this.__sourceSchema.functions[procedure].argTypes,
						this.__sourceSchema.functions[procedure].privileges,
						this.__targetSchema.functions[procedure].privileges
					);
					if (this.__sourceSchema.functions[procedure].owner != this.__targetSchema.functions[procedure].owner)
						this.__tempScripts.push(
							sql.generateChangeProcedureOwnerScript(
								procedure,
								this.__sourceSchema.functions[procedure].argTypes,
								this.__sourceSchema.functions[procedure].owner
							)
						);
				}
			} else {
		...
		if (global.config.options.schemaCompare.dropMissingFunction)
			for (let procedure in this.__targetSchema.functions) {
				...
				if (!this.__sourceSchema.functions.hasOwnProperty(procedure)) this.__tempScripts.push(sql.generateDropProcedureScript(procedure));
	...
	__compareSequences: function () {
		...
		for (let sequence in this.__sourceSchema.sequences) {
			//Get new or changed sequences
			...
			if (renamedOwnedSequence) {
				...
				this.__compareSequenceDefinition(
					sequence,
					this.__sourceSchema.sequences[sequence],
					this.__targetSchema.sequences[renamedOwnedSequence]
				);
				this.__compareSequencePrivileges(
					sequence,
					this.__sourceSchema.sequences[sequence].privileges,
					this.__targetSchema.sequences[renamedOwnedSequence].privileges
				);
			} else if (this.__targetSchema.sequences[sequence]) {
				...
				this.__compareSequencePrivileges(
					sequence,
					this.__sourceSchema.sequences[sequence].privileges,
					this.__targetSchema.sequences[sequence].privileges
				);
			} else {
	...
	__compareSequencePrivileges: function (sequence, sourceSequencePrivileges, targetSequencePrivileges) {
		for (let role in sourceSequencePrivileges) {
			...
			if (targetSequencePrivileges[role]) {
				...
				if (Object.keys(changes).length > 0) this.__tempScripts.push(sql.generateChangesSequenceRoleGrantsScript(sequence, role, changes));
			} else {

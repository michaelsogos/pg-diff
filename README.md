...
```javascript
{
    "development": { //At least one configuration must exists, but you can have many
        ...
        "options": { //This section is mandatory
            ...
            "schemaCompare": {
                "namespaces": ["public", "other-namespace"], <b>//NEW:</b> Simple string containing just one schema name or an array of comma-separated schema names for which retrieve objects to be compared, if assign null or any other invalid object it will retrieve automatically all available schemas from database
